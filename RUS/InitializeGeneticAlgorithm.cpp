...
void GeneticAlgorithm::initializeParameters(double* dataSet, int dataSetLength, int nPopulation, double scaleFactor, double crossingProbability, Ipp64f* thetas,Ipp64f* phi){
	...
	for (int i = 0; i < _nPopulation; i++) {
		...
		_populationParametersOld[i].h1 = (randomDouble(0.04, 0.08));
		...
		_populationParametersOld[i].h5 = (randomDouble(10, 40));
		//_populationParametersOld[i].h5 = 13.397;

		_populationParametersOld[i].h6 = (randomDouble(10, 50));
		...
		_populationParametersOld[i].h9 = randomDouble(1, 100);
		...
		//if ((area_0 + area_half) / 2 > 0.15 && (area_0 + area_half) / 2 < 0.25){ //< 0.25 && area_half >  0.15 && area_half < 0.25) {
			//std::cout << i << "       " << area << std::endl;
			//_populationParametersOld[i].area = (area_0+ area_half)/2;
		    _populationParametersOld[i].area = 0;
